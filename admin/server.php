<?php 
	...
	if (isset($_POST['reg_user'])) {
		...
		if (empty($email)) { array_push($errors, "L'email is requis"); }
		if (empty($password_1)) { array_push($errors, "Le mot de passe est requis"); }

		if ($password_1 != $password_2) {
			array_push($errors, "Les mots de passent ne correspondent pas");
		}

		// register user if there are no errors in the form
		if (count($errors) == 0) {
			$password = md5($password_1);//encrypt the password before saving in the database
			$query = "INSERT INTO tbl_user (user_login, email, user_pwd, rank) 
					  VALUES('$username', '$email', '$password', 'Membre')";
